# ----- Add noise to the synthetic log data -------- #

import numpy
import matplotlib.patches as mpatches

def add_noise_normal(data, v0, std):
    '''
    This function adds noise along the 1D input data using a normal Gaussian distribution for each 
    point along the data set.
    '''
       
    # Define the size of the 1D data
    size = len(data)
    
    # Creat the zero vector such as data
    noise = numpy.zeros_like(data)
    
    # use normal distribution to generate noise 
    noise = numpy.random.normal(v0, std, size)
        
    # Add the noise to the data and return the final output
    return data + noise
